...
// Reporting Mode Only
Json::Value
doLedgerDataReporting(RPC::JsonContext& context)
{    
    ...
    auto [results, status] = 
        context.app.getReportingETL().getReportingBackend().doUpperBound(key, lpLedger->info().seq, limit);

    for (auto const& [hash, entry] : results)
    {  
        SerialIter it(entry->data(), entry->size());
        auto sle = std::make_shared<SLE const>(it, hash);
...
doLedgerData(RPC::JsonContext& context)
{
    if(context.app.config().reporting())
        return doLedgerDataReporting(context);
